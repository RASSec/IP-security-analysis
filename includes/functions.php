...
{
    $db = getPdo();
    $tmp2 = $db->query("select ip as ips,count(*) as cc from data  where isdel=1 group by ip order by count(*) DESC limit 10");
    $data = $tmp2->fetchAll(PDO::FETCH_ASSOC);
    return array(
        'data' => $data
        );
	...
	function gettopipdate()
	{
		$db = getPdo();
		$tmp2 = $db->query("select ip as ips,count(*) as cc from data where TO_DAYS(tt)=TO_DAYS(NOW()) and isdel=1 group by ip order by count(*) DESC limit 10");
		$data = $tmp2->fetchAll(PDO::FETCH_ASSOC);
		return array(
			'data' => $data
		);
	...
	function gettopipweek()
	{
		$db = getPdo();
		$tmp2 = $db->query("select ip as ips,count(*) as cc from data where YEARWEEK(DATE_FORMAT(tt,'%Y-%m-%d'))=YEARWEEK(NOW()) and isdel=1 group by ip order by count(*) DESC limit 10");
		$data = $tmp2->fetchAll(PDO::FETCH_ASSOC);
		return array(
			'data' => $data
		);
	...
	function gettopipmonth()
	{
		$db = getPdo();
		$tmp2 = $db->query("select ip as ips,count(*) as cc from data where DATE_FORMAT(tt,'%Y%m')=DATE_FORMAT(CURDATE(),'%Y%m') and isdel=1  group by ip order by count(*) DESC limit 10");
		$data = $tmp2->fetchAll(PDO::FETCH_ASSOC);
		return array(
			'data' => $data
		);
    ...
    {
        $db = getPdo();
        $tmp2 = $db->query("select rule from rules");
        $data = $tmp2->fetchAll(PDO::FETCH_ASSOC);
        return array(
            'data' => $data
        );
	...
	{
		$db = getPdo();
		$db->query("update rules set rule=CONCAT(rule,\"".$rule."\")");
		//header('Location: /');
